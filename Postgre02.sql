...
WHERE address IS NULL;--Do not use '=NULL'
...
--What are the diffrences between TRUNCATE and DELETE
--1) In DELETE FROM, you can use WHERE clause to filter data, but in TRUNCATE you cannot.
...
--How to get specific multiple field
SELECT name,salary
FROM workers;
...
SELECT  *
FROM workers
WHERE salary IN (2000, 7000, 12000);-- IN() is used for multiple/repeated OR 
